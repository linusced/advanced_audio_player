...
void lc_audio::Engine::thread_func(Engine *instance)
{
    ...
    while (instance->runThread)
    {
        ...
        case THREAD_ACTION_TYPE::PLAY:
        {
            ...
            float gain = (glfwGetTime() - instance->nextThreadAction.time) / instance->nextThreadAction.duration;
            if (gain > 1.0f)
                gain = 1.0f;
        ...
        case THREAD_ACTION_TYPE::PAUSE:
        {
            ...
            float gain = 1.0 - ((glfwGetTime() - instance->nextThreadAction.time) / instance->nextThreadAction.duration);
            src.setGain(gain * instance->volume);
            instance->threadVolumeChange = false;
